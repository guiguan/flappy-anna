...
import (
	...
	ranna "github.com/guiguan/flappy-anna/anna"
	...
	rflappy "github.com/hajimehoshi/ebiten/examples/resources/images/flappy"
...
func init() {
	img, _, err := image.Decode(bytes.NewReader(ranna.Anna_gopher_png))
	if err != nil {
		log.Fatal(err)
	}
	gopherImage, _ = ebiten.NewImageFromImage(img, ebiten.FilterDefault)

	img, _, err = image.Decode(bytes.NewReader(rflappy.Tiles_png))
	if err != nil {
		log.Fatal(err)
	}
...
func (g *Game) Update(screen *ebiten.Image) error {
	...
	switch g.mode {
	case ModeTitle:
		texts = []string{"FLAPPY ANNA", "", "", "", "", "PRESS SPACE KEY", "", "OR TOUCH SCREEN"}
	case ModeGameOver:
		texts = []string{"", "HAPPY QI XI 2018!", "LOVE U"}
	}
	for i, l := range texts {
	...
	if g.mode == ModeTitle {
		msg := []string{
			"Presented to you by Guan",
			"Made in Melb with Love",
		}
		for i, l := range msg {
...
func main() {
	...
	if err := ebiten.Run(g.Update, screenWidth, screenHeight, 1, "Flappy Anna"); err != nil {
		panic(err)
	}
}
